# BYOW Technical Design Document (Phiên bản v8 - Tối ưu hóa Shuffling Phòng thủ & Mã nguồn Chi tiết)
**Tác giả:** Anh Ha  
**Phiên bản:** v8 (Cập nhật chiến lược Shuffling phòng thủ với mã nguồn chi tiết)  
...
Đây là một biện pháp **Thiết kế Phòng thủ (Defensive Design)** nhằm duy trì sự toàn vẹn của tập hợp Frontier `activeRooms`, đồng thời tăng tối đa độ che phủ (density) của bản đồ game.

#### Mã nguồn Java minh họa cụ thể cho chiến lược:
...
public class MapGenerator {
    ...
    // Lớp helper đóng gói kết quả mọc nhánh thành công
    ...
    private BranchResult tryCreateBranch(Room parent, List<Room> allComponents) {
        ...
        // 2. Xáo trộn ngẫu nhiên danh sách hướng
        Collections.shuffle(directions, RANDOM);

        // 3. Duyệt tuần tự qua từng hướng đã xáo trộn
        for (int direction : directions) {
            ...
            // Sinh ứng viên hành lang và phòng con dựa trên các công thức tịnh tiến tọa độ gối biên
            ...
            // 4. Kiểm tra điều kiện biên và va chạm không gian
        ...
        // 5. Thử hết cả 4 hướng đều thất bại -> Bị bủa vây hoàn toàn
        return null;
    }
    ...
    private boolean inBounds(Room r) {
        Position pos = r.getBottomLeft();
        return pos.getX() > 0 && pos.getX() + r.getWidth() < WIDTH - 1
                && pos.getY() > 0 && pos.getY() + r.getHeight() < HEIGHT - 1;
    }
}
```

#### Phân tích luồng xử lý:
1.  **Tính Ngẫu nhiên (Randomized Shuffling)**: Việc bốc ngẫu nhiên hướng giúp loại bỏ sự đơn điệu tuyến tính của hầm ngục, tạo ra các ngã rẽ và bố cục hữu cơ vô cùng bất ngờ cho người chơi.
2.  **Đánh giá Toàn vẹn (Exhaustive Checking)**: Code duyệt qua toàn bộ danh sách `directions`. Nếu một hướng mọc nhánh bị va chạm với các thành phần cũ, thuật toán không vội vàng xóa bỏ `parent`. Nó sẽ kiên nhẫn kiểm tra các hướng còn lại.
3.  **Hạ cánh An toàn (Safe Backtracking)**: Chỉ khi vòng lặp `for` chạy hết mà không hướng nào lọt qua được bước kiểm tra `overlapsAny`, phương thức mới trả về `null`. Khi đó, vòng lặp điều phối chính tại `generate()` mới an tâm gọi `activeRooms.remove(parent)` để loại bỏ "mầm nghẽn", giải phóng không gian rẽ nhánh cho các căn phòng thoáng đãng khác.
