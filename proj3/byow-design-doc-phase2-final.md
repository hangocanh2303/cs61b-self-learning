...
**Tác giả:** Anh Ha
**Phiên bản:** v17 (Bản Thiết Kế Đồng Nhất Tối Hậu - Triệt Tiêu Hoàn Toàn Rò Rỉ Và Inconsistencies)
...
```
                     ...
                     +──────────────────────────────────+
                                      ...
                                      v
                     +──────────────────────────────────+
                     |           Core.Engine            | <─── (Điều phối chính, chứa runGame)
                     +──────────────────────────────────+
                                      │
         ┌────────────────────────────┴────────────────────────────┐
...
+──────────────────+                                        +──────────────────+
         ...
         └────────────────────────────┬────────────────────────────┘
                     ...
                     +──────────────────────────────────+
                        │              │              │
                        ├─ TETile[][]  ├─ Position    ├─ int health
                        ├─ Random rng  ├─ score       └─ StringBuilder inputHistory
```
...
Trong chế độ GUI trực quan, chúng ta tuyệt đối không được sử dụng cơ chế đọc chặn (blocking) vì nó sẽ làm đóng băng toàn bộ luồng cập nhật HUD khi người chơi đứng im di chuột [326]. Chúng ta sử dụng kiểm tra không chặn qua `hasNextKeyTyped()` [326]:
...
### 2.3 Lớp `RandomInputSource` (Mô phỏng Stress Testing / Fuzzing)
...
## 3. Lớp GameState & Cơ chế Va chạm Vật lý Phòng ngự
...
public class GameState implements Serializable {
     ...
     * Toàn bộ ô gạch nằm ngoài khoảng này sẽ hiển thị là Tileset.NOTHING (270 Điểm Ambition - Primary Feature)
     */
...
## 4. Hiện thực hóa Lớp Điều phối Engine.java - Máy Trạng Thế Đồng Nhất (Unified State Machine Loop)

Để giải quyết triệt để lỗi DRY và Temporal Decomposition, `Engine.java` sẽ sử dụng duy nhất một máy trạng thái trung tâm chạy trên luồng đa hình `InputSource` [28.3]:
...
public class Engine {
    ...
    private GameState runGame(InputSource source, boolean render) {
        ...
        // Duyệt tuần tự qua dòng ký tự nhận được từ nguồn input đa hình [312]
...
Sử dụng toán học Chebyshev rời rạc để lọc và vẽ một vùng không gian chiếu sáng hình vuông bán kính bằng `5` ô gạch bọc xung quanh người chơi. Toàn bộ ô gạch nằm ngoài khoảng này sẽ hiển thị là `Tileset.NOTHING` [315]. Cơ chế này đã được đóng gói cực kỳ sâu và gọn gàng thành phương thức member **`gameState.getRenderFrame()`** bên trong `GameState.java` (như trình bày chi tiết tại Mục 3) nhằm tránh rò rỉ ranh giới hiển thị ra bên ngoài [28.3].
...
## 8. Chiến lược Kiểm thử Tích hợp (Gradescope JUnit Integration Testing)

Để tự tin kiểm chứng tính tất định (Determinism) của `Engine` trước khi nộp bài lên Gradescope, chúng ta xây dựng bộ kiểm thử tích hợp (Integration Tests) trực tiếp bằng JUnit 4 chuẩn mực. Trực tiếp mô phỏng các trường hợp chạy liên hoàn tạo mới thế giới, lưu trữ (:Q) và khôi phục (L) để đảm bảo thế giới sinh ra là trùng khớp 100% [322, 342].
...
public class BYOWIntegrationTest {
    ...
    @Test
    public void testDeterministicMovement() {
        Engine engine = new Engine();
        
        // Luồng 1: Tạo thế giới với hạt giống 999 và di chuyển trực tiếp liên tục
        TETile[][] worldDirect = engine.interactWithInputString("N999SDDDWWWDDD");
        
        // Luồng 2: Tạo thế giới hạt giống 999, đi sang phải (DDD), lưu game (:Q)
        engine.interactWithInputString("N999SDDD:Q");
        // Sau đó tải lại game (L), đi lên (WWW), đi sang phải (DDD)
        TETile[][] worldResume = engine.interactWithInputString("LWWWDDD");
        
        // CHỐT CHẶN KIỂM THỬ: worldDirect và worldResume phải giống hệt nhau từng ô gạch!
        assertTrue("Trạng thái thế giới sau khi khôi phục không đồng nhất với đi trực tiếp!", 
                   Arrays.deepEquals(worldDirect, worldResume));
    }
    ...
    public void testConsecutiveSaveAndLoad() {
        ...
        // Chạy chuỗi phím lưu/tải liên tục qua nhiều phiên
        engine.interactWithInputString("N12345SDDD:Q");
        engine.interactWithInputString("LWWW:Q");
        engine.interactWithInputString("LAAA:Q");
        TETile[][] worldSplit = engine.interactWithInputString("L");
        
        // So khớp với chuỗi đi liên tục từ đầu
        TETile[][] worldContinuous = engine.interactWithInputString("N12345SDDDWWWAAA");
        
        assertTrue("Chuỗi lưu tải liên hoàn gây bất đồng bộ trạng thái thế giới!", 
                   Arrays.deepEquals(worldContinuous, worldSplit));
    }
}
```
