...
**Tác giả:** Anh Ha
**Phiên bản:** v17 (Bản Thiết Kế Hoàn Chỉnh Đồng Nhất Tối Hậu - Sẵn Sàng Hiện Thực Hóa 100%)
...
Để giải quyết triệt để lỗi DRY và Temporal Decomposition, `Engine.java` sử dụng duy nhất một máy trạng thái trung tâm chạy trên luồng đa hình `InputSource` [28.3] và vẽ Menu chào mừng ngay khi người dùng khởi chạy chế độ Keyboard:
...
public class Engine {
    ...
    /**
     * Vẽ màn hình Menu chính (Main Menu) đẹp mắt và trực quan.
     */
    private void drawMainMenu() {
        StdDraw.clear(StdDraw.BLACK);
        StdDraw.setPenColor(StdDraw.WHITE);
        
        // 1. Vẽ Tiêu đề game lớn
...
Sử dụng toán học Chebyshev rời rạc để lọc và vẽ một vùng không gian chiếu sáng hình vuông bán kính bằng `5` ô gạch bọc xung quanh người chơi. Toàn bộ ô gạch nằm ngoài khoảng này sẽ hiển thị là `Tileset.NOTHING` [315]. Hành vi đã được đóng gói trực tiếp trong `getRenderFrame` của lớp `GameState` tại Mục 3 [315].
...
## 8. Chiến lược Kiểm thử Tích hợp (BYOW Integration Testing)

Để tự tin kiểm chứng tính tất định (Determinism) của `Engine` trước khi nộp bài lên Gradescope, chúng ta sử dụng bộ khung kiểm thử tích hợp (Integration Tests) trực tiếp qua JUnit 4, loại bỏ hoàn toàn cơ chế kiểm thử rườm rà của các đồ án khác [336]:
...
public class BYOWIntegrationTest {
    ...
    public void testDeterministicMovement() {
        ...
        // Chạy hai thế giới độc lập với cùng hạt giống và chuỗi di chuyển
        TETile[][] world1 = engine1.interactWithInputString("N12345SWWWAAASD:Q");
        TETile[][] world2 = engine2.interactWithInputString("N12345SWWWAAASD:Q");
        
        // Đảm bảo kết quả trả về là hoàn toàn giống hệt nhau (Tính tất định)
    ...
    public void testSaveAndLoadPersistence() {
        ...
        // Hai mảng gạch lưới cuối cùng phải khớp hoàn hảo từng ô gạch!
        assertArrayEquals(world1, world2);
    }
}
```
